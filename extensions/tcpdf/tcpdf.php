...
class TCPDF
{
			...
			if (isset($format['trans'])) {
				...
				if (isset($format['trans']['S']) and in_array($format['trans']['S'], $stansition_styles)) {
					...
					if (isset($format['trans']['Di']) and in_array($format['trans']['S'], $valid_effect)) {
						if (
							((($format['trans']['Di'] == 90) or ($format['trans']['Di'] == 180)) and ($format['trans']['S'] == 'Wipe'))
	...
	public function Annotation($x, $y, $w, $h, $text, $opt = array('Subtype' => 'Text'), $spaces = 0)
	{
		...
		if (!$this->pdfa_mode) {
			if (
				(($opt['Subtype'] == 'FileAttachment') or ($opt['Subtype'] == 'Sound')) and (!TCPDF_STATIC::empty_string($opt['FS']))
			...
			if ($c == 10) { // 10 = "\n" = new line
				...
				if (($l > $wmax) or (($c == 173) and (($l + $tmp_shy_replacement_width) >= $wmax))) {
					...
					if ($sep == -1) {
						...
						if (
							($this->rtl and ($this->x <= ($this->w - $this->rMargin - $this->cell_padding['R'] - $margin['R'] - $chrwidth)))
	...
	public function Image($file, $x = '', $y = '', $w = 0, $h = 0, $type = '', $link = '', $align = '', $resize = false, $dpi = 300, $palign = '', $ismask = false, $imgmask = false, $border = 0, $fitbox = false, $hidden = false, $fitonpage = false, $alt = false, $altimgs = array())
	{
		...
		if ($newimage) {
			...
			if ((method_exists('TCPDF_IMAGES', $mtd)) and (!($resize and (function_exists($gdfunction) or extension_loaded('imagick'))))) {
				...
				if (
					($ismask === false) and ($imgmask === false) and (strpos($file, '__tcpdf_' . $this->file_id . '_imgmask_') === FALSE)
	...
	public function write1DBarcode($code, $type, $x = '', $y = '', $w = '', $h = '', $xres = '', $style = '', $align = '')
	{
		...
		if ($style['stretch']) {
			...
			if (
				(isset($style['padding']) and ($style['padding'] === 'auto'))
		...
		while ($elkey < $maxel) {
			...
			if (preg_match($tagpattern, $element)) {
				...
				if ($element[0] == '/') {
					...
					if (
						(($dom[$key]['value'] == 'ul') or ($dom[$key]['value'] == 'ol') or ($dom[$key]['value'] == 'dl'))
	...
	public function writeHTML($html, $ln = true, $fill = false, $reseth = false, $cell = false, $align = '')
	{
		...
		while ($key < $maxel) {
			...
			if ($dom[$key]['tag'] and isset($dom[$key]['attribute']['pagebreak'])) {
				...
				if (
					(($dom[$key]['attribute']['pagebreak'] == 'left') and (((!$this->rtl) and (($this->page % 2) == 0)) or (($this->rtl) and (($this->page % 2) != 0))))
			...
			if ($dom[$key]['tag'] or ($key == 0)) {
				...
				if ((!$this->newline) and ($dom[$key]['value'] == 'img') and (isset($dom[$key]['height'])) and ($dom[$key]['height'] > 0)) {
					...
					if (!empty($dom[$key]['width'])) {
						...
						if (
							($imgw <= ($this->w - $this->lMargin - $this->rMargin - $this->cell_padding['L'] - $this->cell_padding['R']))
					...
					if (
						($fontname != $curfontname) or ($fontstyle != $curfontstyle) or ($fontsize != $curfontsize)
			...
			if ($this->newline and (strlen($dom[$key]['value']) > 0) and ($dom[$key]['value'] != 'td') and ($dom[$key]['value'] != 'th')) {
				...
				if (
					!($dom[$key]['tag'] and !$dom[$key]['opening'] and ($dom[$key]['value'] == 'table')
	...
	protected function openHTMLTagHandler($dom, $key, $cell)
	{
		...
		if ($dom[$key]['self'] and isset($dom[$key]['attribute']['pagebreakafter'])) {
			...
			if (
				(($pba == 'left') and (((!$this->rtl) and (($this->page % 2) == 0)) or (($this->rtl) and (($this->page % 2) != 0))))
	...
	protected function closeHTMLTagHandler($dom, $key, $cell, $maxbottomliney = 0)
	{
		...
		switch ($tag['value']) {
			case 'tr': {
				...
				if ($this->num_columns > 1) {
					if (
						(($this->current_column == 0) and ($dom[($dom[$key]['parent'])]['endcolumn'] == ($this->num_columns - 1)))
	...
	protected function startSVGElementHandler($parser, $name, $attribs, $ctm = array())
	{
		...
		switch ($name) {
			...
			case 'linearGradient': {
				...
				if (
					((!isset($attribs['x1'])) and (!isset($attribs['y1'])) and (!isset($attribs['x2'])) and (!isset($attribs['y2'])))
			...
			case 'radialGradient': {
				...
				//$attribs['spreadMethod']
				if (
					((!isset($attribs['cx'])) and (!isset($attribs['cy'])))
